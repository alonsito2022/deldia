...
import com.sys4soft.deldia.adapter.ProductSaleAdapter
...
//            .fit()
    ...
    private fun validateSale(): Boolean {
        ...
        // Validar que solo se puedan generar facturas si el cliente tiene DNI
        if (autoCompleteDocumentType.text.toString() == "FACTURA" && person.documentTypeDisplay != "RUC") {
            Toast.makeText(globalContext, "Solo se puede emitir factura a clientes con RUC", Toast.LENGTH_SHORT).show()
            return false
        }
